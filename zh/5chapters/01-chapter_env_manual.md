...
例如工程目录为F:\git_repositories\ART_wifi\firmware\app，先进入工程根目录。
...
使用menuconfig来配置项目所需要的组件包，然后通过pkgs --update命令来更新项目中的组件包。如果不想要某个组件包，可以在menuconfig的配置中去掉包选项，然后再次使用pkgs --update命令更新即可。
