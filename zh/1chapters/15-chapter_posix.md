...
**函数原型**    ```c int pthread_atfork(void (*prepare)(void), void (*parent)(void), void (*child)(void)); ```

**函数返回**    返回EOPNOTSUPP。

**函数原型**    ```c int pthread_kill(pthread_t thread, int sig); ```

**函数返回**    返回PTHREAD_SCOPE_SYSTEM。

**函数原型**    ```c int pthread_spin_init (pthread_spinlock_t *lock, int pshared); ```

**函数返回**    lock无效返回EINVAL，否则返回0。

**函数原型**    ```c int pthread_spin_destroy (pthread_spinlock_t *lock); ```

**函数返回**    lock无效返回EINVAL，否则返回0。

**函数原型**    ```c int pthread_spin_lock (pthread_spinlock_t * lock); ```

**函数返回**    lock无效返回EINVAL，否则返回0。

**函数原型**    ```c int pthread_spin_trylock (pthread_spinlock_t * lock); ```

**函数返回**    lock无效返回EINVAL，否则返回0或EBUSY。

**函数原型**    ```c int pthread_spin_unlock (pthread_spinlock_t * lock); ```
...
**函数原型**  int pthread_rwlockattr_getpshared (const pthread_rwlockattr_t *attr, int *pshared);
-----------------------------------------------------------------------
        参数    描述
--------------  -------------------------------------------------------
        attr	指向读写锁属性的指针
...
**函数原型**  int pthread_barrierattr_getpshared(const pthread_barrierattr_t *attr, int *pshared);
-----------------------------------------------------------------------
        参数    描述
--------------  -------------------------------------------------------
        attr	指向屏障属性的指针
