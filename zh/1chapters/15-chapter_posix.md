...
### 发送取消请求
  ...
  头文件     \#include\<pthread.h\>
  ---------- -----------------------------------------
  函数原形   int pthread\_cancel(pthread\_t thread);
  参数       描述
  thread     线程句柄
  返回值     只返回0值，总是成功
...
| 头文件                            | \#include\<pthread.h\>            |
+===================================+===================================+
| 函数原形                          | int pthread\_setcancelstate(int   |
|                                   | state, int \*oldstate);           |
+-----------------------------------+-----------------------------------+
| 参数                              | 描述                              |
+-----------------------------------+-----------------------------------+
...
+===================================+===================================+
| 函数原形                          | int pthread\_setcanceltype(int    |
|                                   | type, int \*[[]{#OLE_LINK15       |
|                                   | .anchor}]{#OLE_LINK14             |
|                                   | .anchor}oldtype);                 |
+-----------------------------------+-----------------------------------+
...
### 设置取消点
  ...
  头文件     \#include\<pthread.h\>
  ---------- ---------------------------------
  函数原形   void pthread\_testcancel(void);
...
------------
  ...
  头文件          \#include\<pthread.h\>
  --------------- ----------------------------------------------------------------------------------------------------------------------------------
  函数原形        int [[]{#OLE_LINK7 .anchor}]{#OLE_LINK6 .anchor}pthread\_once(pthread\_once\_t \* once\_control, void (\*init\_routine) (void));
  参数            描述
  once\_control   控制变量
  init\_routine   执行函数
  返回值          只返回0值，总是成功
...
| 头文件   | \#include \<sched.h\>                                         |
+==========+===============================================================+
| 函数原形 | int sched\_get\_priority\_min(int policy);                    |
|          |                                                               |
...
------------------
  ...
  头文件     \#include\<pthread.h\>
  ---------- -----------------------------------------------------------------------------------------------------------------------------------------
  函数原形   int [[]{#OLE_LINK92 .anchor}]{#OLE_LINK78 .anchor}pthread\_atfork(void (\*prepare)(void), void (\*parent)(void), void (\*child)(void));
  返回值     EOPNOTSUPP

  头文件     \#include\<pthread.h\>
  ---------- ------------------------------------------------
  函数原形   int pthread\_kill(pthread\_t thread, int sig);
  返回值     PTHREAD\_SCOPE\_SYSTEM

+-----------------------------------+-----------------------------------+
| 头文件                            | \#include\<pthread.h\>            |
+===================================+===================================+
| 函数原形                          | int pthread\_spin\_init           |
...
+-----------------------------------+-----------------------------------+
  ...
  头文件     \#include\<pthread.h\>
  ---------- ---------------------------------------------------------
  函数原形   int pthread\_spin\_lock (pthread\_spinlock\_t \* lock);
  返回值     lock无效返回EINVAL，否则返回0

  头文件     \#include\<pthread.h\>
  ---------- ------------------------------------------------------------
  函数原形   int pthread\_spin\_trylock (pthread\_spinlock\_t \* lock);
  返回值     lock无效返回EINVAL，否则返回0或EBUSY

  头文件     \#include\<pthread.h\>
  ---------- -----------------------------------------------------------
  函数原形   int pthread\_spin\_unlock (pthread\_spinlock\_t \* lock);
...
------------
  ...
  头文件     \#include\<pthread.h\>
  ---------- -----------------------------------------------------------
  函数原形   int pthread\_condattr\_init(pthread\_condattr\_t \*attr);
  ...
  头文件     \#include\<pthread.h\>
  ---------- -------------------------------------------------------------------------------------
  函数原形   int pthread\_mutexattr\_getpshared(pthread\_mutexattr\_t \*attr, int \*pshared);
...
### 条件变量属性相关桩函数
  ...
  头文件     \#include\<pthread.h\>
  ---------- --------------------------------------------------------------
  函数原形   int pthread\_condattr\_destroy(pthread\_condattr\_t \*attr);
...
+-----------------------------------+-----------------------------------+
  ...
  头文件     \#include\<pthread.h\>
  ---------- --------------------------------------------------------------------------------
  函数原形   int pthread\_mutexattr\_setpshared(pthread\_mutexattr\_t \*attr, int pshared);
...
----------
  ...
  头文件     \#include\<pthread.h\>
  ---------- ----------------------------------------------------------------
  函数原形   int pthread\_rwlockattr\_init (pthread\_rwlockattr\_t \*attr);
  ...
  头文件     \#include\<pthread.h\>
  ---------- -------------------------------------------------------------------------------------------
  函数原形   int pthread\_rwlockattr\_getpshared (const pthread\_rwlockattr\_t \*attr, int \*pshared);
  返回值     参数无效返回EINVAL，否则返回0，pshared指向的内存保存的值为PTHREAD\_PROCESS\_PRIVATE
...
### 读写锁属性相关桩函数
  ...
  头文件     \#include\<pthread.h\>
  ---------- -----------------------------------------------------------------------------------
  函数原形   int pthread\_rwlockattr\_setpshared (pthread\_rwlockattr\_t \*attr, int pshared);
  返回值     参数无效返回EINVAL，否则返回0值

+-----------------------------------+-----------------------------------+
| 头文件                            | \#include\<pthread.h\>            |
+===================================+===================================+
| 函数原形                          | int pthread\_rwlockattr\_destroy  |
...
--------
  ...
  头文件     \#include\<pthread.h\>
  ---------- -----------------------------------------------------------------
  函数原形   int pthread\_barrierattr\_init(pthread\_barrierattr\_t \*attr);
  ...
  头文件     \#include\<pthread.h\>
  ---------- ------------------------------------------------------------------------------------
  函数原形   int pthread\_barrierattr\_setpshared(pthread\_barrierattr\_t \*attr, int pshared);
  功能       获取屏障的作用域
  参数       描述
  attr       指向屏障属性的指针
  ...
  头文件     \#include\<pthread.h\>
  ---------- --------------------------------------------------------------------------------------------
  函数原形   int pthread\_barrierattr\_getpshared(const pthread\_barrierattr\_t \*attr, int \*pshared);
  功能       获取屏障的作用域
  参数       描述
  attr       指向屏障属性的指针
...
| 头文件   | \#include\<mqueue.h\>                              |
+==========+====================================================+
| 函数原形 | Int mq\_setattr(mqd\_t mqdes,                      |
|          |                                                    |
...
+----------+----------------------------------------------------+
  ...
  头文件     \#include\<mqueue.h\>
  ---------- ----------------------------------------------------------
  函数原形   int mq\_getattr(mqd\_t mqdes, struct mq\_attr \*mqstat);
  功能       获取消息队列属性，
  参数       描述
  mqdes      指向消息队列控制块的指针
  mqstat     指向保存获取数据的指针
