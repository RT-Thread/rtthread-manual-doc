...
做为一个例子，例6-1中详细分析了STM32F103的串口驱动，也包括上层应该如何使用这个设备的代码。STM32F103串口驱动代码，详细的中文注释已经放在其中了。
...
对于包含中断发送、接收的情况的驱动程序，例6-2给出了具体的使用代码。在这个例子中，用户线程将在两个设备上(UART1, UART2)读取数据，然后再写到UART1设备中。
